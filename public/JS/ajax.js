...
const postAPI = async (peliculas) => {
    try {
		/* validador de json: https://es.piliapp.com/json/validator/*/
...
const updateAPI = async (peliculas) => {
	...
	try {
            ...
            alert("Ha ocurrido un error, por lo que no se ha podido actualizar la información");
            return;
          }
}
...
const indexView = (peliculas) => {
    ...
    view += `<div class="actions">
				...
				<button class="new">A\xF1adir</button>
...
const deleteContr = async (i) => {
	...
	if (confirm(`Esta seguro que desea eliminar la pelicula: ${mis_peliculas[i].titulo}`)) {
                mis_peliculas.splice(i, 1); // elimina elemento i del array correspondiente al pelicula
                await updateAPI(mis_peliculas);
                await indexContr();
            }
}
...
const resetContr = async () => {
	...
	mis_peliculas = [...mis_peliculas_iniciales];  //clono las peliculas iniciales
    await updateAPI(mis_peliculas);
    await indexContr();
}
